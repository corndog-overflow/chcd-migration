...
input_file = 'chcd_v2.3_nodes.csv'
output_file = 'nodes_fix.csv'

with open(input_file, 'r') as file:
    reader = csv.reader(file, delimiter='@')
    rows = list(reader)

# Fix the format of each row
for row in rows:
...
df.to_csv('nodes_fix.csv', index=False)

input_file = 'chcd_v2.3_edges.csv'
output_file = 'rels_fix.csv'
...
print(df)
df.to_csv('rels_fix.csv', index=False)